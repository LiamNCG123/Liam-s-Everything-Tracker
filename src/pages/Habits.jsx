      ...
      <td className="sticky left-0 z-10 bg-theme-card pr-3 py-1.5 min-w-[120px] max-w-[160px]">
        <div className="flex items-center gap-2">
          ...
          <div className="flex flex-col min-w-0">
            <span className="text-sm font-medium text-theme-primary truncate leading-tight">
              {habit.name}
            </span>
            {habit.cue && (
...
function SlotHeader({ slot, habits, todayStr }) {
  ...
  return (
    <tr>
      ...
      <td colSpan={999} className="bg-theme-input border-t border-theme" />
    </tr>
  )
}
...
function ColorPicker({ value, onChange }) {
  return (
    <div>
      <span className="text-sm font-medium text-theme-secondary block mb-2">Color</span>
...
function GoalPicker({ value, onChange, goals }) {
  ...
  return (
    <div>
      <span className="text-sm font-medium text-theme-secondary block mb-1">Contributing to goal (optional)</span>
      <select
        value={value}
        onChange={e => onChange(e.target.value)}
        className="w-full border border-theme rounded-xl px-3 py-2 text-sm text-theme-primary focus:outline-none focus:ring-2 focus:ring-brand-500 bg-theme-input focus:bg-white dark:focus:bg-gray-700 transition"
      >
...
function TimeOfDayPicker({ value, onChange }) {
  return (
    <div>
      <span className="text-sm font-medium text-theme-secondary block mb-2">Time of day</span>
    ...
    <div>
        ...
        <>
          ...
          <div className="flex items-center gap-4 mb-3 px-1">
            <div className="text-sm text-theme-muted">
              <span className="font-semibold text-theme-primary">{doneToday}</span>
              <span>/{habits.length} done today</span>
            </div>
            {topStreak > 0 && (
              <div className="text-sm text-theme-muted">
                🔥 <span className="font-semibold text-theme-primary">{topStreak}</span> day top streak
              </div>
            )}
          </div>
          ...
          <div className="flex items-center justify-between mb-3 px-1">
            <button
              onClick={prevMonth}
              className="text-theme-muted hover:text-gray-700 dark:hover:text-gray-200 px-2 py-1 rounded-lg hover:bg-gray-100 hover:bg-theme-hover transition text-lg"
            >
              ‹
            </button>
            <span className="text-sm font-semibold text-theme-secondary">{monthLabel}</span>
            <button
              onClick={nextMonth}
              className="text-theme-muted hover:text-gray-700 dark:hover:text-gray-200 px-2 py-1 rounded-lg hover:bg-gray-100 hover:bg-theme-hover transition text-lg"
          ...
          <div className="flex items-center gap-3 mt-3 px-1">
            <span className="text-xs text-theme-muted">Less</span>
            {[0.15, 0.4, 0.65, 0.85, 1].map(o => (
              <div key={o} className="w-3 h-3 rounded-sm" style={{ backgroundColor: `rgba(99,102,241,${o})` }} />
            ))}
            <span className="text-xs text-gray-400">More</span>
            <span className="text-xs text-gray-300 text-theme-muted ml-2">· Click any cell to toggle</span>
          </div>
        </>
      )}
      ...
      >
        <div className="flex flex-col gap-4">
          ...
          {/* Preview */}
          {form.name && (
            <div className="flex items-center gap-2 p-3 bg-theme-input rounded-xl">
              <span className="w-3 h-3 rounded-full shrink-0" style={{ backgroundColor: form.color }} />
              <div className="flex flex-col min-w-0">
                <span className="text-sm font-medium text-theme-primary">{form.name}</span>
                {form.cue && (
