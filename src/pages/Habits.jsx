...
function HabitRow({ habit, days, todayStr, onToggle, onEdit, onDelete, goalTitle }) {
  ...
  return (
    <tr className="group">
      ...
      <td className="sticky left-0 z-10 bg-white dark:bg-dm-card pr-3 py-1.5 min-w-[120px] max-w-[160px]">
        <div className="flex items-center gap-2">
          ...
          <div className="flex flex-col min-w-0">
            <span className="text-sm font-medium text-gray-800 dark:text-dm-primary truncate leading-tight">
              {habit.name}
            </span>
            {habit.cue && (
      ...
      <td className="sticky right-0 z-10 bg-white dark:bg-dm-card pl-3 py-1.5 whitespace-nowrap">
        {(() => {
          ...
          const streakColor = current >= 30
            ...
            : 'text-gray-300 dark:text-dm-muted'
          ...
          return (
            <div className="flex flex-col gap-0.5 items-end">
              ...
              <div className="text-[10px] text-gray-400 dark:text-dm-muted leading-none">
                {atRisk
      ...
      <td className="pl-2">
        <div className="flex gap-1">
          <button
            ...
            className="text-xs text-gray-500 dark:text-dm-muted hover:text-gray-800 dark:hover:text-gray-100 px-2 py-1 rounded hover:bg-gray-100 dark:hover:bg-dm-hover"
          >
            Edit
          </button>
          <button
...
function SlotHeader({ slot, habits, todayStr }) {
  ...
  return (
    <tr>
      <td className="sticky left-0 z-10 bg-gray-50 dark:bg-dm-input px-3 pt-3.5 pb-1 min-w-[120px] max-w-[160px] border-t border-gray-100 dark:border-dm-border">
        <div className="flex items-center gap-1.5">
          <span className="text-sm leading-none">{slot.icon}</span>
          <span className="text-xs font-semibold text-gray-500 dark:text-dm-muted uppercase tracking-wide leading-none">
            {slot.label}
          </span>
      ...
      <td colSpan={999} className="bg-gray-50 dark:bg-dm-input border-t border-gray-100 dark:border-dm-border" />
    </tr>
  )
}
...
function ColorPicker({ value, onChange }) {
  return (
    <div>
      <span className="text-sm font-medium text-gray-700 dark:text-dm-secondary block mb-2">Color</span>
...
function GoalPicker({ value, onChange, goals }) {
  ...
  return (
    <div>
      <span className="text-sm font-medium text-gray-700 dark:text-dm-secondary block mb-1">Contributing to goal (optional)</span>
      <select
        value={value}
        onChange={e => onChange(e.target.value)}
        className="w-full border border-gray-200 dark:border-dm-border rounded-xl px-3 py-2 text-sm text-gray-900 dark:text-dm-primary focus:outline-none focus:ring-2 focus:ring-brand-500 bg-gray-50 dark:bg-dm-input focus:bg-white dark:focus:bg-gray-700 transition"
      >
...
function TimeOfDayPicker({ value, onChange }) {
  return (
    <div>
      <span className="text-sm font-medium text-gray-700 dark:text-dm-secondary block mb-2">Time of day</span>
    ...
    <div>
      ...
      {habits.length === 0 ? (
        ...
        <>
          ...
          <div className="flex items-center gap-4 mb-3 px-1">
            <div className="text-sm text-gray-500 dark:text-dm-muted">
              <span className="font-semibold text-gray-900 dark:text-dm-primary">{doneToday}</span>
              <span>/{habits.length} done today</span>
            </div>
            {topStreak > 0 && (
              <div className="text-sm text-gray-500 dark:text-dm-muted">
                🔥 <span className="font-semibold text-gray-900 dark:text-dm-primary">{topStreak}</span> day top streak
              </div>
            )}
          </div>
          ...
          <div className="flex items-center justify-between mb-3 px-1">
            <button
              onClick={prevMonth}
              className="text-gray-400 dark:text-dm-muted hover:text-gray-700 dark:hover:text-gray-200 px-2 py-1 rounded-lg hover:bg-gray-100 dark:hover:bg-dm-hover transition text-lg"
            >
              ‹
            </button>
            <span className="text-sm font-semibold text-gray-700 dark:text-dm-secondary">{monthLabel}</span>
            <button
              onClick={nextMonth}
              className="text-gray-400 dark:text-dm-muted hover:text-gray-700 dark:hover:text-gray-200 px-2 py-1 rounded-lg hover:bg-gray-100 dark:hover:bg-dm-hover transition text-lg"
          ...
          {/* Calendar grid */}
          <div className="overflow-x-auto rounded-2xl border border-gray-100 dark:border-dm-subtle bg-white dark:bg-dm-card shadow-sm">
            <table className="border-collapse" style={{ tableLayout: 'fixed' }}>
              <thead>
                <tr>
                  <th className="sticky left-0 z-20 bg-white dark:bg-dm-card min-w-[120px] max-w-[160px]" />
                  {days.map(({ day, dateStr }) => (
                    ...
                    >
                      <div className={`text-[10px] font-semibold w-7 mx-auto leading-none pt-2 pb-0.5 ${
                        dateStr === todayStr ? 'text-brand-600' : 'text-gray-400 dark:text-dm-muted'
                      }`}>
                        {day}
                      </div>
                      <div className={`text-[9px] w-7 mx-auto leading-none pb-2 ${
                        dateStr === todayStr ? 'text-brand-400' : 'text-gray-300 dark:text-dm-muted'
                      }`}>
                        {DAY_LABELS[new Date(viewYear, viewMonth, day).getDay()]}
                      </div>
                    </th>
                  ))}
                  <th className="sticky right-0 z-20 bg-white dark:bg-dm-card min-w-[64px]">
                    <div className="text-[10px] font-semibold text-gray-400 dark:text-dm-muted text-right pr-3 pt-2 pb-2">
                      Streak
                    </div>
                  </th>
              ...
              <tbody className="divide-y divide-gray-50 dark:divide-dm-subtle">
                {useGroups
                  ? grouped.map(slot => (
          ...
          <div className="flex items-center gap-3 mt-3 px-1">
            <span className="text-xs text-gray-400 dark:text-dm-muted">Less</span>
            {[0.15, 0.4, 0.65, 0.85, 1].map(o => (
              <div key={o} className="w-3 h-3 rounded-sm" style={{ backgroundColor: `rgba(99,102,241,${o})` }} />
            ))}
            <span className="text-xs text-gray-400">More</span>
            <span className="text-xs text-gray-300 dark:text-dm-muted ml-2">· Click any cell to toggle</span>
          </div>
        </>
      )}
      ...
      >
        <div className="flex flex-col gap-4">
          ...
          {/* Preview */}
          {form.name && (
            <div className="flex items-center gap-2 p-3 bg-gray-50 dark:bg-dm-input rounded-xl">
              <span className="w-3 h-3 rounded-full shrink-0" style={{ backgroundColor: form.color }} />
              <div className="flex flex-col min-w-0">
                <span className="text-sm font-medium text-gray-800 dark:text-dm-primary">{form.name}</span>
                {form.cue && (
