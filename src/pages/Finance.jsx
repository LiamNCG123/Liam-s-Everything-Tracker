...
const EXPENSE_CATS = [
  'Rent/Housing','Utilities','Groceries','Eating Out','Transport',
  'Health','Fitness','Shopping','Entertainment','Travel',
  'Business','Taxes','Subscriptions','Miscellaneous',
]
const INCOME_CATS = [
...
function calcInsights(transactions, budgets, month) {
  ...
  const overBudget = budget
    ? budget.items.filter(b => {
        const spent = byCat.find(([c]) => c === b.category)?.[1] || 0
...
function BudgetTab({ transactions, budgets, month, onSaveBudget }) {
  const budget = budgets.find(b => b.month === month)
  const byCat  = Object.fromEntries(calcCategorySpend(transactions, month))
  const [editing, setEditing] = useState(false)
  const [items, setItems] = useState(() => budget?.items || [])

  const startEdit = () => {
    setItems(budget?.items ? [...budget.items] : EXPENSE_CATS.map(c => ({ category: c, limit: '' })))
    setEditing(true)
  }

  const save = () => {
    onSaveBudget({ month, items: items.filter(i => i.limit !== '' && Number(i.limit) > 0) })
    setEditing(false)
  }

  const totalBudget = (budget?.items || []).reduce((s, i) => s + Number(i.limit || 0), 0)
  const totalSpent  = (budget?.items || []).reduce((s, i) => s + (byCat[i.category] || 0), 0)
  const overCount   = (budget?.items || []).filter(i => (byCat[i.category] || 0) > Number(i.limit)).length

  if (editing) {
    return (
      <div>
        <div className="flex items-center justify-between mb-4">
          <h2 className="font-bold text-gray-900">Set Budget — {monthLabel(month)}</h2>
          <div className="flex gap-2">
            <Button variant="secondary" size="sm" onClick={() => setEditing(false)}>Cancel</Button>
            <Button size="sm" onClick={save}>Save</Button>
          </div>
        </div>
        <p className="text-xs text-gray-400 mb-4">Leave blank to skip a category.</p>
        <div className="flex flex-col gap-2">
          {items.map((item, i) => (
            <div key={item.category} className="flex items-center gap-3 bg-white rounded-xl px-3 py-2 border border-gray-100">
              <span className="w-2.5 h-2.5 rounded-full shrink-0" style={{ backgroundColor: catColor(item.category) }} />
              ...
              <div className="relative w-28">
                ...
                <input
                  ...
                  value={item.limit}
                  onChange={e => setItems(it => it.map((x, j) => j === i ? { ...x, limit: e.target.value } : x))}
                  className="w-full pl-5 pr-2 py-1.5 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-1 focus:ring-brand-500 bg-gray-50"
                />
              </div>
            </div>
          ))}
    ...
    <div>
      <div className="flex items-center justify-between mb-4">
        <div>
          ...
          {overCount > 0 && (
            <p className="text-xs text-red-500 mt-0.5">{overCount} categor{overCount > 1 ? 'ies' : 'y'} over budget</p>
          )}
        </div>
      ...
      <Card className="p-4 mb-4">
        ...
        <ProgressBar
          value={totalSpent} max={totalBudget}
      ...
      <div className="flex flex-col gap-2">
        {budget.items.map(item => {
          const spent = byCat[item.category] || 0
          const limit = Number(item.limit)
          const pct   = limit > 0 ? (spent / limit) * 100 : 0
          const over  = spent > limit
          const near  = !over && pct >= 80
          return (
            <Card key={item.category} className={`p-4 ${over ? 'border-red-100' : ''}`}>
              <div className="flex items-center justify-between mb-2">
                <div className="flex items-center gap-2">
                  <span className="w-2.5 h-2.5 rounded-full" style={{ backgroundColor: catColor(item.category) }} />
                ...
                <div className="flex items-center gap-2">
                  ...
                  {!over && !near && spent > 0 && <Badge color="green">On track</Badge>}
                </div>
              </div>
              <ProgressBar
                value={spent} max={limit}
    ...
    <div className="flex flex-col gap-5">
      ...
      <div>
        ...
        <div className="flex flex-col gap-2">
          ...
          {ins.largest && (
            <InsightRow icon="🏆" label="Largest expense category"
              value={`${ins.largest[0]} (${fmt$(ins.largest[1])})`} />
          )}
          {ins.spendTrend !== null && (
            <InsightRow
          ...
          {ins.overBudget.length > 0 && (
            <InsightRow icon="⚠️" label="Over budget"
