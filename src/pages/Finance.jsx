  ...
  return (
    <Modal open={open} onClose={onClose} title={initial ? 'Edit Transaction' : 'Add Transaction'}>
      <div className="flex flex-col gap-4">
        ...
        <div className="flex gap-1 bg-theme-input rounded-xl p-1">
            ...
            <button key={t} onClick={() => set('type', t)}
              className={`flex-1 py-1.5 rounded-lg text-sm font-semibold capitalize transition-colors ${
                form.type === t
                  ...
                  : 'text-theme-muted hover:text-gray-700 dark:hover:text-gray-300'
              }`}
            >{t}</button>
          ))}
        </div>
        ...
        {/* Amount */}
        <div>
          <span className="text-sm font-medium text-theme-secondary block mb-1">Amount</span>
          <div className="relative">
            <span className="absolute left-3 top-1/2 -translate-y-1/2 text-theme-muted font-medium">$</span>
            <input
              type="number" min="0" step="0.01" placeholder="0.00"
              value={form.amount} onChange={e => set('amount', e.target.value)}
              autoFocus
              className="w-full pl-7 pr-3 py-3 text-xl font-bold border border-theme rounded-xl focus:outline-none focus:ring-2 focus:ring-brand-500 bg-theme-input text-theme-primary placeholder-theme-muted focus:bg-theme-input"
            />
          </div>
        </div>
        ...
        <div>
          <span className="text-sm font-medium text-theme-secondary block mb-2">Category</span>
          <div className="max-h-40 overflow-y-auto">
            {form.type === 'expense' ? (
              CATEGORY_GROUPS.map(g => (
                <div key={g.group} className="mb-2">
                  <div className="text-[10px] font-semibold text-theme-muted uppercase tracking-wide mb-1 px-0.5">{g.group}</div>
                  <div className="flex flex-wrap gap-1.5">
                    {g.categories.map(c => (
                      <button key={c.name} onClick={() => { set('category', c.name); setCatLocked(true); setSuggestion(null) }}
                        className={`px-3 py-1 rounded-full text-xs font-medium transition-colors ${
                          form.category === c.name ? 'text-white shadow-sm' : 'bg-theme-input text-theme-secondary hover:bg-gray-200 hover:bg-theme-hover'
                        }`}
              ...
              <div className="flex flex-wrap gap-1.5">
                {INCOME_CATS.map(c => (
                  <button key={c} onClick={() => { set('category', c); setCatLocked(true); setSuggestion(null) }}
                    className={`px-3 py-1 rounded-full text-xs font-medium transition-colors ${
                      form.category === c ? 'text-white shadow-sm' : 'bg-theme-input text-theme-secondary hover:bg-gray-200 hover:bg-theme-hover'
                    }`}
        ...
        <label className="flex items-center gap-2 cursor-pointer">
          ...
          <span className="text-sm text-theme-secondary">Recurring transaction</span>
        </label>
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      <div className="grid grid-cols-3 gap-3">
        ...
        <Card className={`p-4 ${net >= 0 ? 'bg-green-50 dark:bg-green-900/30 border-green-100' : 'bg-red-50 dark:bg-red-900/30 border-red-100'}`}>
          <div className={`text-2xl font-bold leading-tight ${net >= 0 ? 'text-green-700' : 'text-red-600'}`}>
            {net >= 0 ? '+' : ''}{fmt(net)}
          </div>
          <div className="text-sm font-medium text-theme-secondary mt-0.5">Net</div>
        </Card>
      </div>
      ...
      <Card className="p-4">
        <div className="flex justify-between text-sm mb-2">
          <span className="font-semibold text-theme-secondary">Month progress</span>
          <span className="text-theme-muted">{pct}% through month</span>
        </div>
        <ProgressBar value={pct} color="indigo" />
        <div className="flex justify-between mt-3 text-sm">
          <span className="text-theme-muted">Projected end-of-month net</span>
      ...
      {byCat.length > 0 && (
        <div>
          <h3 className="text-sm font-semibold text-theme-muted uppercase tracking-wide mb-2">Top spending</h3>
                ...
                <div key={cat}>
                  <div className="flex justify-between text-sm mb-1">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="font-medium text-theme-primary">{cat}</span>
                    </div>
                    <div className="flex items-center gap-2">
                      <span className="text-theme-secondary font-semibold">{fmt(amt)}</span>
                      {budgetItem && (
                        <span className={`text-xs ${amt > budgetItem.limit ? 'text-red-500' : 'text-theme-muted'}`}>
      ...
      {recent.length > 0 && (
        <div>
          <h3 className="text-sm font-semibold text-theme-muted uppercase tracking-wide mb-2">Recent</h3>
          <Card className="divide-y divide-theme-subtle">
            {recent.map(t => (
              <div key={t.id} className="flex items-center gap-3 px-4 py-3">
                ...
                <div className="flex-1 min-w-0">
                  <div className="text-sm font-medium text-theme-primary truncate">{t.description}</div>
                  <div className="text-xs text-theme-muted">{t.category} · {fmtDate(t.date)}</div>
                </div>
    ...
    <div>
      ...
      <div className="flex flex-col gap-2 mb-4">
        <input
          ...
          className="w-full border border-theme rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500 bg-theme-input text-theme-primary placeholder-theme-muted"
        />
        <div className="flex gap-2 overflow-x-auto no-scrollbar">
          {['all','income','expense'].map(t => (
            <button key={t} onClick={() => setTypeFilter(t)}
              className={`shrink-0 px-3 py-1 rounded-full text-xs font-medium capitalize transition-colors ${
                typeFilter === t ? 'bg-brand-500 text-white' : 'bg-theme-input text-theme-secondary hover:bg-gray-200 hover:bg-theme-hover'
              }`}>{t}</button>
          ))}
          <div className="w-px bg-gray-200 bg-theme-hover shrink-0" />
          {allCats.map(c => (
            <button key={c} onClick={() => setCatFilter(catFilter === c ? 'all' : c)}
              className={`shrink-0 px-3 py-1 rounded-full text-xs font-medium transition-colors ${
                catFilter === c ? 'text-white' : 'bg-theme-input text-theme-secondary hover:bg-gray-200 hover:bg-theme-hover'
              }`}
            ...
            <div key={t.id} className="flex items-center gap-3 px-4 py-3 group">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-1.5 flex-wrap">
                  <span className="text-sm font-medium text-theme-primary">{t.description}</span>
                  {t.recurring && <span className="text-[10px] text-theme-muted">🔄 {t.recurringFrequency}</span>}
                </div>
              ...
              <div className="flex items-center gap-2 shrink-0">
                ...
                <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                  <button onClick={() => onEdit(t)} className="text-xs text-theme-muted hover:text-gray-700 dark:hover:text-gray-300 px-1 py-0.5 rounded hover:bg-gray-100 hover:bg-theme-hover">Edit</button>
      ...
      {visible.length > 0 && (
        <div className="flex justify-between mt-3 px-1 text-sm">
          <span className="text-theme-muted">{visible.length} transactions</span>
    ...
    return (
      <div>
        <div className="flex items-center justify-between mb-3">
          <h2 className="font-bold text-theme-primary">Budget — {monthLabel(month)}</h2>
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-4">
        <div>
          <h2 className="font-bold text-theme-primary">{monthLabel(month)}</h2>
...
function InsightsTab({ transactions, budgets, month }) {
  ...
  return (
    <div className="flex flex-col gap-5">
      {/* Forecast card */}
      <Card className="p-4">
        <h3 className="font-bold text-theme-primary mb-1">End-of-month forecast</h3>
        ...
        <div className="grid grid-cols-3 gap-3">
          <div className="text-center">
            ...
            <div className="text-xs text-theme-muted mt-0.5">Proj. income</div>
          </div>
          <div className="text-center">
            <div className="text-lg font-bold text-red-500">{fmt(fc.projExpense)}</div>
            <div className="text-xs text-theme-muted mt-0.5">Proj. expenses</div>
          ...
          <div className="text-center">
            ...
            <div className="text-xs text-theme-muted mt-0.5">Proj. net</div>
          </div>
        </div>
        {totalBudget > 0 && (
          <div className="mt-4">
      ...
      {ins.expense > 0 && (
        <div>
          <h3 className="text-sm font-semibold text-theme-muted uppercase tracking-wide mb-3">Spending breakdown</h3>
          <Card className="p-4 flex flex-col gap-4">
            {/* Fixed vs Variable */}
            <div>
              <div className="flex justify-between text-xs text-theme-muted mb-1.5">
                <span className="font-medium text-theme-secondary">Fixed vs Variable</span>
                <span>{fmt(ins.fixedSpend)} fixed · {fmt(ins.variableSpend)} variable</span>
              </div>
              <div className="flex h-2.5 rounded-full overflow-hidden bg-theme-input">
                <div
              ...
              <div className="flex gap-3 mt-1.5 text-[10px] text-theme-muted">
            ...
            <div>
              <div className="flex justify-between text-xs text-theme-muted mb-1.5">
                <span className="font-medium text-theme-secondary">Essentials vs Lifestyle</span>
      ...
      {/* Key insights */}
      <div>
        <h3 className="text-sm font-semibold text-theme-muted uppercase tracking-wide mb-3">Key insights</h3>
        <div className="flex flex-col gap-2">
      ...
      {Object.keys(ins.byGroup).length > 0 && (
        <div>
          <h3 className="text-sm font-semibold text-theme-muted uppercase tracking-wide mb-3">Spend by group</h3>
            ...
            {CATEGORY_GROUPS.map(g => {
              ...
              return (
                <div key={g.group} className="flex items-center gap-3">
                  <span>{g.icon}</span>
                  <span className="flex-1 text-sm text-theme-secondary">{g.group}</span>
                  <span className="text-sm font-bold text-theme-primary">{fmt(amt)}</span>
      ...
      {ins.top3.length > 0 && (
        <div>
          <h3 className="text-sm font-semibold text-theme-muted uppercase tracking-wide mb-3">Top expense categories</h3>
          <Card className="p-4 flex flex-col gap-3">
            {ins.top3.map(([cat, amt], i) => (
              <div key={cat} className="flex items-center gap-3">
                <span className="text-lg font-bold text-gray-200 text-theme-muted w-5 shrink-0">{i + 1}</span>
                <span className="w-2.5 h-2.5 rounded-full shrink-0" style={{ backgroundColor: catColor(cat) }} />
                <span className="flex-1 text-sm font-medium text-theme-secondary">{cat}</span>
                <span className="text-sm font-bold text-theme-primary">{fmt(amt)}</span>
              </div>
            ))}
          </Card>
...
function InsightRow({ icon, label, value, color }) {
  const colors = { green: 'text-green-600', red: 'text-red-500', default: 'text-theme-primary' }
  return (
    <div className="flex items-center justify-between bg-theme-card rounded-xl px-4 py-3 border border-theme-subtle">
      <div className="flex items-center gap-2">
        <span>{icon}</span>
  ...
  return (
    ...
    <div>
      ...
      <div className="flex items-center justify-between mb-4 flex-wrap gap-2">
        <h1 className="text-3xl font-bold tracking-tight text-theme-primary">Finance</h1>
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <div className="flex items-center gap-1">
            <select
              ...
              className="text-xs border border-theme rounded-lg px-2 py-1.5 bg-theme-card text-theme-secondary focus:outline-none focus:ring-2 focus:ring-indigo-300"
            >
            ...
            <select
              ...
              className="text-xs border border-theme rounded-lg px-2 py-1.5 bg-theme-card focus:outline-none focus:ring-2 focus:ring-indigo-300 font-semibold text-indigo-700 dark:text-indigo-400"
            >
              {CURRENCIES.map(c => <option key={c} value={c}>{c}</option>)}
            </select>
            {rateLoading && <span className="text-[10px] text-theme-muted animate-pulse">loading…</span>}
            {rateError   && <span className="text-[10px] text-red-400" title={rateError}>rates unavailable</span>}
          </div>

          <div className="flex items-center gap-1">
            <button onClick={prevMonth}
              className="text-theme-muted hover:text-gray-700 dark:hover:text-gray-300 px-2 py-1 rounded-lg hover:bg-gray-100 hover:bg-theme-hover transition text-lg">‹</button>
            <span className="text-sm font-semibold text-theme-secondary w-28 text-center">{monthLabel(month)}</span>
            <button onClick={nextMonth} disabled={isCurrentMonth}
              className="text-theme-muted hover:text-gray-700 dark:hover:text-gray-300 disabled:opacity-30 px-2 py-1 rounded-lg hover:bg-gray-100 hover:bg-theme-hover transition text-lg">›</button>
          </div>
        </div>
      </div>

      {/* Tabs */}
      <div className="flex gap-1 bg-theme-input rounded-xl p-1 mb-5 overflow-x-auto no-scrollbar">
        {TABS.map(({ key, label }) => (
